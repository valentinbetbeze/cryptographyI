...
int main(int argc, char *argv[])
{
    if (argc != 6)
    {
        fprintf(
            stderr,
            "Usage: %s decrypt <key_len> <key_hex> <nonce_hex> <data_hex>\n",
            argv[0]);
        fprintf(
            stderr,
    ...
    // Only support decrypt for now (CTR mode: encrypt and decrypt are the same
    // operation)
    ...
    // Parse key
    const size_t key_byte_len = key_bits / 8;
    ...
    // Parse nonce/IV (16 bytes / 4 words)
    uint8_t nonce_bytes[16];
    uint32_t nonce_words[4];
    if (hex_to_bytes(nonce_hex, nonce_bytes, 16) != 0)
    {
    ...
    bytes_to_words(nonce_bytes, nonce_words, 16);

    // Parse ciphertext (variable length)
    const size_t ct_len = strlen(data_hex) / 2;
    uint8_t *ct_bytes   = malloc(ct_len);
    uint32_t *ct_words  = malloc(ct_len + sizeof(uint32_t));
    ...
    // Perform decryption (in CTR mode, decrypt and encrypt are the same)
    uint32_t *result = malloc(ct_len + sizeof(uint32_t));
    int ret = aes_ctr_decrypt(nonce_words, ct_words, ct_len, result, &key);

    if (ret != 0)
    {
        fprintf(stderr, "Decryption failed with error code: %x\n", ret);
        free(key_bytes);
        free(key_words);
        free(ct_bytes);
        free(result);
